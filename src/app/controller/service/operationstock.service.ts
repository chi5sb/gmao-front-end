...
export class OperationstockService {
  ...
  findAll(): void {
    this.http.get<Array<OperationStock>>(this.urlBase + this.url + '/').subscribe(
      data => {
 ...
 save() {
    this.http.post(this.urlBase + this.url + '/', this.operationstock).subscribe(
      data => {
        if (data === -1){
          alert('l un d est reference entre n est pas disponible dans la base de donnees');
        }
        if (data === -2){
  ...
  myClone.magasinDestination.reference = operationstock.magasinDestination.reference;
  ...
  find(reference: String) {
    this.http.get<Array<OperationStock>>(this.urlBase + this.url + '/trouve/refmagasin/' + reference).subscribe(
      data => {
        console.log(data);
        this.operationstocks = data;
      }
